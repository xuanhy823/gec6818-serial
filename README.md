...
| `serial-term.ps1` | 终端仿真：按字节收发、UTF-8/GBK、方向键、Ctrl+C |
| `serial-ios.ps1` | 界面控件（圆角卡片、按钮） |
| `serial-transfer-core.ps1` | 串口打开、恢复 shell、传文件、启动板上程序 |
...
- **串口**：选 CH340；点「刷新」重新扫描
- **波特率**：`9600 / 19200 / 38400 / 57600 / 115200 / 230400 / 460800 / 921600`，开发板默认 **115200**
- **文件传输 | 串口终端**：切换页面
- **连接 / 断开 / 清屏 / Ctrl+C**：终端操作（在顶栏，避免被黑框挡住）
- 右上角状态：空闲 / 已连接 / 传输中

传输和终端**不能同时占口**。终端连着时先「断开」再传输。

### 串口终端

用来在板上敲 Linux 命令，验证文件在不在、进程起没起。

1. 选好串口和 115200，点黑色 **连接**
2. 鼠标点进中间黑色区域，直接打字（板子回显，类似 MobaXterm）
3. **Enter** 执行；**方向键**走板上历史（ash 自己的历史）
4. **Ctrl+C**：没选中文字时发给板子中断；有选中则复制
5. **右键**：有选中复制，否则粘贴
6. 底部快捷按钮会发 `uname -a`、`ls -l /home`、`df -h /home`、`pidof vehicle_course`、`cat /tmp/vc.log`、`free`
7. 用完点 **断开**，把口让给文件传输或 MobaXterm
...
4. 可选：
   - **再拷到 /usr/local/bin**：重启后还在系统路径里
   - **传完自动启动**
5. 点 **开始传输**，看进度条。115200 下大约 **11 KB/s**，约 284KB 要 **30–80 秒**，属正常
6. 成功后再点 **启动板上程序**（或勾了自动启动则不用再点）
...
| 现象 | 处理 |
|------|------|
| `open_fail` / 访问被拒绝 | 关 MobaXterm 串口标签、本工具重复窗口 |
